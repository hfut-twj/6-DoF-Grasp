...
def get_loss(end_points):
    ...
    collision_loss, end_points = compute_collision_loss(end_points)
    score_loss, end_points = compute_score_loss(end_points)
    width_loss, end_points = compute_width_loss(end_points)
    loss = objectness_loss + 10 * graspness_loss + 100 * view_loss + 15 * score_loss + 10 * width_loss + collision_loss    # + best_view_loss
    end_points['loss/overall_loss'] = loss
    end_points['loss/overall_loss_source'] = objectness_loss + 10 * graspness_loss + 100 * view_loss + 15 * score_loss + 10 * width_loss
...
def compute_score_loss(end_points):
    ...
    device = grasp_score_pred.device
...
def compute_collision_loss(end_points):
    criterion = nn.BCEWithLogitsLoss(reduction='mean')
    grasp_collision_pred = end_points['grasp_collision_pred'].float()  # grasp_collision_pred torch.Size([2, 2, 1024, 12, 4])
    device = grasp_collision_pred.device
    grasp_collision_label = end_points['batch_grasp_collision'].unsqueeze(1).contiguous()    # grasp_collision_label torch.Size([2, 1024, 12, 4])
    grasp_collision_label_111 = (~ grasp_collision_label)
    grasp_collision_label = grasp_collision_label.float()
    grasp_collision_label_111 = grasp_collision_label_111.float()
    grasp_collision_label = torch.cat([grasp_collision_label, grasp_collision_label_111], 1)
    loss = criterion(grasp_collision_pred, grasp_collision_label)
    end_points['loss/stage3_collision_loss'] = loss
    return loss, end_points